...
import joblib
# load
modello = joblib.load("heatmap.pkl")
...
while ret:
    ...
    cv2.imshow("image",squared_image)
    cv2.waitKey(1)
    if (time.time() - inizio) > 14:
        print("tempo passato")
        break
    cv2.imshow("heatmap", heatmapmod)
plt.imshow(heatmapmod, cmap='hot', interpolation='nearest')
plt.axis('off') 
plt.savefig('heatmap.jpg', bbox_inches='tight', pad_inches=0)  

from skimage.transform import resize
heatmapmod=imread('heatmap.jpg')
img_resize=resize(heatmapmod,(150,150,3))
l=[img_resize.flatten()]
probability=modello.predict_proba(l)
Categories=['CornerKick','GoalKick','KickIn','Goal','PushingFreeKick','Substitution','FullTime']

for ind,val in enumerate(Categories):
    print(f'{val} = {probability[0][ind]*100}%')
print("The predicted image is : "+Categories[modello.predict(l)[0]])